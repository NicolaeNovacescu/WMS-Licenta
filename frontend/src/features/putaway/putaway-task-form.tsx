...
import type { PutawayWorkflowFormState } from "@/types/putaway";
...
export function PutawayTaskForm({
  action,
  sourceBalances,
  destinationLocations,
}: PutawayTaskFormProps) {
    ...
    return (
      <section className="rounded-[28px] border border-line bg-white/84 p-6 shadow-[0_18px_70px_rgba(29,41,56,0.08)] backdrop-blur">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-accent">
          {putawayMessages.form.emptyEyebrow}
        </p>
        <p className="mt-4 text-sm leading-7 text-muted">
          {putawayMessages.form.emptyMessage}
        </p>
      </section>
    );
  }
    ...
    <section className="rounded-[28px] border border-line bg-white/84 p-6 shadow-[0_18px_70px_rgba(29,41,56,0.08)] backdrop-blur">
      ...
      <div className="mt-5 rounded-2xl border border-line bg-surface px-4 py-4">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-warning">
          {putawayMessages.form.meaningEyebrow}
        </p>
        <p className="mt-3 text-sm leading-6 text-ink">
          {putawayMessages.form.meaningDescription}
        </p>
      </div>

      <form action={formAction} className="mt-6 space-y-5">
        <Field label={putawayMessages.form.sourceBalanceLabel} htmlFor="sourceBalanceId">
          <select
            id="sourceBalanceId"
          ...
          >
            {sourceBalances.map((balance) => (
              <option key={balance.id} value={balance.id}>
                ...
                {interpolateMessage(putawayMessages.form.availableTemplate, {
        ...
        {selectedSource ? (
          <>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              />
              <SummaryCard
                label={putawayMessages.form.availableToMoveLabel}
                value={formatDisplayQuantity(selectedSource.availableQuantity, locale)}
                caption={interpolateMessage(
                  putawayMessages.form.quantitySummaryTemplate,
                  {
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field
                label={putawayMessages.form.destinationLocationLabel}
              ...
              >
                ...
                >
                  {validDestinations.length === 0 ? (
                    <option value="">
                      {putawayMessages.form.noValidDestinations}
                    </option>
                  ) : (
              ...
              <Field label={putawayMessages.form.quantityLabel} htmlFor="quantity">
                <input
        ...
        <Field label={putawayMessages.form.notesLabel} htmlFor="notes">
          <textarea
            id="notes"
            name="notes"
            rows={4}
            className={`${inputClassName} min-h-28 resize-y`}
            placeholder={putawayMessages.form.notesPlaceholder}
          />
        </Field>
        ...
        {selectedSource && validDestinations.length === 0 ? (
          <div className="rounded-2xl border border-amber-300 bg-warning-soft px-4 py-3 text-sm text-ink">
            {putawayMessages.form.noValidDestinationMessage}
          </div>
        ) : null}
        ...
        <SubmitButton
          label={putawayMessages.form.createSubmit}
          pendingLabel={putawayMessages.form.pending}
