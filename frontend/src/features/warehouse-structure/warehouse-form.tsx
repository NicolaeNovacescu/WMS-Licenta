  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2">
        <InputField
          label={setupMessages.forms.warehouseCode}
          name="code"
          placeholder="MAIN"
          defaultValue={warehouse?.code ?? ""}
        />
        <InputField
          label={setupMessages.forms.warehouseName}
          name="name"
      ...
      <label className="flex items-center gap-3 rounded-2xl border border-line bg-surface px-4 py-3 text-sm text-ink">
        ...
        />
        {setupMessages.forms.warehouseIsActive}
      </label>
      ...
      <SubmitButton
        label={
          mode === "create"
            ? setupMessages.forms.createWarehouse
            : setupMessages.forms.saveWarehouse
        }
