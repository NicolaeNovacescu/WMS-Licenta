...
import { PutawayTaskDetailPage } from "@/features/putaway/putaway-task-detail-page";
import { getRequestLocale } from "@/lib/i18n/get-request-locale";
import { getMessages } from "@/lib/i18n/messages";
import { getPutawayTask } from "@/lib/api/putaway-api";
import { getPageAccess, hasRole } from "@/lib/navigation/route-access";

type PutawayTaskDetailRouteProps = {
  params: Promise<{
    id: string;
  }>;
...
export default async function PutawayTaskDetailRoute({
  params,
  searchParams,
}: PutawayTaskDetailRouteProps) {
  const locale = await getRequestLocale();
  const messages = getMessages(locale);
  const { id } = await params;
  const resolvedSearchParams = await searchParams;
  const access = await getPageAccess(`/putaway-tasks/${id}`);
  ...
  const putawayTaskResult = await getPutawayTask(id);

  if (!putawayTaskResult.ok) {
    if (putawayTaskResult.status === 404) {
      notFound();
    }
    ...
    return (
      <ApiErrorState
        ...
        title={messages.putawayTasks.route.detailUnavailableTitle}
        message={putawayTaskResult.message}
        fallbackMessage={messages.putawayTasks.route.detailUnavailableFallback}
      />
    );
  }

  return (
    <PutawayTaskDetailPage
      putawayTask={putawayTaskResult.data}
